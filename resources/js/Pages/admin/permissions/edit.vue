<template>
          ...
          <div class="mt-6 p-2 bg-slate-100">
            ...
            <div class="flex space-x-2 mt-4 p-2">
              <template v-if="form.roles">
                <form v-for="role in form.roles" :key="role.id" class="px-4 py-2 bg-red-500 hover-bg-red-700 text-white rounded-md" @submit.prevent="removeRole(role.id)">
                  @csrf
                  @method('DELETE')
            ...
            <span v-if="form.errors.role" class="text-red-400 text-sm">{{ form.errors.role[0] }}</span>
              </div>
              <div class="sm:col-span-6 pt-5">
  ...
  export default {
    ...
    methods: {
      submitForm() {
        this.$inertia.put(route('admin.permissions.update', this.$page.props.permission.id), this.form, {
          onSuccess: () => {
      ...
      assignRole() {
        this.$inertia.post(route('admin.permissions.roles', this.$page.props.permission.id), { role: this.selectedRole }, {
          onSuccess: () => {
            this.$inertia.visit(route('admin.permissions.index'));
          },
          onError: (errors) => {
      ...
      removeRole(roleId) {
        if (confirm('Are you sure?')) {
          this.$inertia.delete(route('admin.permissions.roles.remove', [this.$page.props.permission.id, roleId]), {
            onSuccess: () => {
              this.$inertia.visit(route('admin.permissions.index'));
            },
          });
